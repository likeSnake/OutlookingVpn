...
class HttpsTest : ViewModel() {
    sealed class Status {
        ...
        companion object {
            const val CODE_IDLE = -1
            const val CODE_TESTING = 0
        ...
        open fun retrieve(setStatus: (CharSequence, Int) -> Unit, errorCallback: (String) -> Unit) = setStatus(status, code)
        ...
        sealed class Error : Status() {
            ...
            override fun retrieve(setStatus: (CharSequence, Int) -> Unit, errorCallback: (String) -> Unit) {
            ...
            class UnexpectedResponseCode(private val codeError: Int) : Error() {
                override val error get() = app.getString(R.string.connection_test_error_status_code, codeError)
            }
